...
{{- default "${application-name}" .Values.nameOverride | trunc 63 | trimSuffix "-" }}
{{- end }}
...
{{- else }}
{{- $name := default "${application-name}" .Values.nameOverride }}
...
{{- printf "%s-%s" "${application-name}" .Chart.Version | replace "+" "_" | trunc 63 | trimSuffix "-" }}
{{- end }}
...
app: {{ include "${application-name}.name" . }}
{{- if .Chart.AppVersion }}
...
Pod labels
*/}}
{{- define "${application-name}.podLabels" -}}
{{ include "${application-name}.selectorLabels" . }}
app: {{ include "${application-name}.name" . }}
{{- if .Chart.AppVersion }}
version: {{ .Chart.AppVersion | quote }}
